...
// Helper function to generate default CDR extensions for outbound routes
const generateDefaultExtensions = (route, routeId) => {
  return [
    {
...
export const createOutboundRoute = async (req, res) => {
  ...
  try {
    ...
    // Create default CDR extensions using the helper function
    const defaultExtensions = generateDefaultExtensions(route, route.id);
...
export const updateOutboundRoute = async (req, res) => {
  ...
  try {
    ...
    if (applications !== undefined) {
      // Delete ALL generated extensions (both default CDR and app-generated)
      await VoiceExtension.destroy({
        where: {
          outboundRouteId: routeId,
          isGenerated: true,
      ...
      // Always recreate default CDR extensions with updated context/phoneNumber
      const defaultExtensions = generateDefaultExtensions(
        { context: mainRouteData.context || route.context, phoneNumber: mainRouteData.phoneNumber || route.phoneNumber },
        routeId
      );
      await VoiceExtension.bulkCreate(defaultExtensions, { transaction });
